...
def reduce_mem_usage(df, verbose=False):
    ...
    for col in float_columns: ch for ch in featureLi]
...
def add_demand_features(df):
    ...
    df["rolling_skew_t30"]= df.groupby([
        "id"
    ])["demand"].transform(lambda x: x.shift(DAYS_PRED).rolling(30).skew())
    df["rolling_kurt_t30"]=df.groupby([
        "id"
    ])["demand"].transform(lambda x: x.shift(DAYS_PRED).rolling(30).kurt())
    return df
...
prices = encode_categorical(prices,
...
aadata = add_price_features(data).pipe(reduce_mem_usage)
dt_col = "date"
...
models = train_lgb(bst_params,
                   fit_params,
                   X_train,
                   y_train,
                   cv,
                   drop_when_train=[day_col])
